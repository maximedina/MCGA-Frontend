...
export class ProvinciaComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.authenticate.isLoggedIn()) {
      this.permissionCreate =
        this.securityService.getPermissionLS("CREAR_PROVINCIA");
      this.permissionDelete =
        this.authenticate.getPermissionLS("BORRAR_PROVINCIA");
  ...
  searchProvincias(form) {
    ...
    this.provinciaService
      .getProvincia(false, form.value.nombre)
      .subscribe((result) => {
        setTimeout(() => {
          this.loading = false;
  ...
  addNewProvincia(provincia: Provincia) {
    this.blockUI.start();
    this.provinciaService.addProvincia(provincia).subscribe(
      (result) => {
        if (result == null) {
          this.provinciaService.getProvincia(false, "").subscribe((result) => {
            this.provincias = result;
  ...
  updateProvincia(provincia: Provincia, index: number) {
    this.blockUI.start();
    this.provinciaService.updateProvincia(provincia).subscribe(
      (result) => {
        if (result == null) {
  ...
  deleteProvincia(index: number) {
    ...
    this.provinciaService.deleteProvincia(provincia).subscribe(
      (result) => {
        if (result == null) {
