...
export class CiudadComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.ciudadService.getProvincias().subscribe((data) => {
  ...
  search(form) {
    this.ciudadService
      .getCiudad(false, form.value.nombre, form.value.provincia)
      .subscribe((result) => {
  ...
  addNewCiudad(ciudad: Ciudad) {
    this.ciudadService.addCiudad(ciudad).subscribe(
      (result) => {
        if (result == null) {
          this.messageService.add({
            ...
            severity: "success",
            summary: "Éxito",
            detail: "Ciudad guardada con éxito.",
          });
          this.ciudadService.getCiudades().subscribe((data) => {
            this.ciudades = data;
          });
    ...
    this.ciudadService.updateCiudad(ciudad).subscribe(
      () => {
        ...
        this.messageService.add({
          ...
          severity: "success",
          summary: "Éxito",
          detail: "Ciudad actualizada con éxito.",
        });
      },
      (error: HttpErrorResponse) => {
        if (error.status === 400 || error.status === 404) {
